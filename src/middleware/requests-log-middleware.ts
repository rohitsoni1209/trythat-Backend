...
import { tryStringify } from '../_app/utils';
...
export class RequestsLogMiddleware implements NestMiddleware {
  ...
  private logRequest(request: Request): void {
    ...
    defaultEnd.apply;
    this.logger.log({
      reqId: getCorrelationId(request) || randomUUID(),
  ...
  private logResponse(response: Response): void {
    ...
    this.logResponseEndCallback(response, () => {
      ...
      this.logFormatResponse(response, requestTimeMillis);
    });
    this.logResponseWriteGetChunks(chunks, response);
    this.logResponseEndGetChunks(chunks, response);
  }

  private logFormatResponse(response: Response, requestTimeMillis: number): void {
  ...
  private isBlackList(url: string, blackList: string[]): boolean {
    ...
    const regex = new RegExp(blackList.map((path) => `^${escapeRegExp(path)}`).join('|'));
    return regex.test(url);
  }

  private filterDataResponse(data: string, url): string {
    return this.isBlackList(url, this.blackListResponseData) ? '*too long*' : data;
  }
  ...
  private logResponseWriteGetChunks(chunks: Buffer[], response: Response): void {
    const defaultWrite = response.write.bind(response);

    response.write = function write(chunk: string | Buffer, ...args: never[]): boolean {
  ...
  private logResponseEndGetChunks(chunks: Buffer[], response: Response): void {
    ...
    response.end = function end(chunk: unknown, ...args: never): Response<any, Record<string, any>> {
      if (typeof chunk === 'string' || Buffer.isBuffer(chunk)) {
        chunks.push(Buffer.from(chunk));
      }
  ...
  private logResponseEndCallback(response: Response, callback: () => void): void {
    const defaultEnd = response.end.bind(response);

    response.end = function end(...args: never): Response<any, Record<string, any>> {
      callback();
